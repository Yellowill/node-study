		...
		Calendar.prototype={
			...
			creat:function(args){
				...
				if(this.ops.date.systemTime!=null){
					this.today = M.formatDate(this.getDateInfo(new Date(this.ops.date.systemTime)).year)+this.timeConnector+M.formatDate(this.getDateInfo(new Date(this.ops.date.systemTime)).month)+this.timeConnector+M.formatDate(this.getDateInfo(new Date(this.ops.date.systemTime)).day);
				}else{
					this.today = M.formatDate(this.getDateInfo().year)+this.timeConnector+M.formatDate(this.getDateInfo().month)+this.timeConnector+M.formatDate(this.getDateInfo().day);
				}
			...
			selectMonth:function(e){
				...
				var _this = e.data.context,
					...
					if(_this.choseNewYear !=''){
						_this.cacheDate =new Date(_this.choseNewYear+_this.timeConnector+newMonth+'-01');
					}else{
						_this.cacheDate =new Date(newYear+_this.timeConnector+newMonth+'-01');
					}
					_this.choseMonthBox.addClass('hide');
					_this.choseYearBox.children().remove();
			...
			fastQueryYear:function(e){
				...
				if(_this.choseNewYear !=''){
					t =new Date(_this.choseNewYear+_this.timeConnector+(_this.cacheDate.getMonth()+1)+_this.timeConnector+'01');
				}else{
					t = _this.cacheDate
				}
				var target=M(this),
...
//					e.data.main.remove()
			...
			creatYear:function(index,list){
				...
				if(this.choseNewYear !=''){
					 date=this.getDateInfo(new Date(this.choseNewYear+this.timeConnector+M.formatDate(this.cacheDate.getMonth()+1)+this.timeConnector+'01'));
				}else{
...
//				}
			...
			tabList:function(){
				...
				for(var i=0; i<list.length;i++){
					var className = (i===0? 'today':'active')
			...
			creatDay:function(index){
				...
				for(var j=1;j<current.week;j++){
					arr.push({
						...
						disabled:true,
						index:j-1,
						id:M.formatDate(prev.year)+this.timeConnector+M.formatDate(prev.month)+this.timeConnector+M.formatDate(prev.length-current.week+j+1),	
						type:index.toString()
					});
				};
				for(var i=1;i<=current.length;i++){
					arr.push({
						...
						type:index.toString(),
						id:M.formatDate(current.year)+this.timeConnector+M.formatDate(current.month)+this.timeConnector+M.formatDate(i)
					});
				};
				for(var z=1;z<=42-current.length-current.week+1;z++){
					var d = new Date(current.year+this.timeConnector+(current.month+1)+this.timeConnector+M.formatDate(z));
					arr.push({
						value:z,
						disabled:true,
						index:current.length+current.week-2+z,
						type:index.toString(),
						id:M.formatDate((d).getFullYear())+this.timeConnector+M.formatDate((d).getMonth()+1)+this.timeConnector+M.formatDate(z)
					});
				};
				this.lastWeek = current.length - (7-(42-current.length-current.week+1)%7);
				this.index = 0;
				this.cacheDate = new Date(''+M.formatDate(current.year)+this.timeConnector+M.formatDate(current.month)+this.timeConnector+'01');
				return {
					year:current.year,
					month:current.month,
