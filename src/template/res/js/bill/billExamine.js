require(['signMessage','head', 'menu', 'base', 'tab', 'page', 'calendar', 'confirm','customDialog', 'status', 'qrcode'],
    function (signMessage) {
        M.define('billIssue', {
            ...
            //复核
            check: function (billNo, id, receivingId,payerIsGroup) {
                var that = this;
                //提交
                that.isClick = false;
                that.signStatus = 0;
                    ...
                    callback: function (e) {
                        ...
                        M.ajaxFn({
                            ...
                            success:function(data){
                                ...
                                M('#passedUserName').html(own.fetch('userInfo').userName);
                        ...
                        M.ajaxFn({
                            ...
                            data: {
                                ...
                                customerId: receivingId
                            },
                            dataType: 'json',
                            success: function (res) {
                                if ( res.success ) {
                                    if (res.data.troeePath && res.data.troeePath.length > 0) {
                                        M.downloadFileXhrImg(res.data.troeePath, '', M('#createSign'));
                                    } else {
                                        M('#createSign').hide();
                                    }

                                    if (res.data.treeIdPath && res.data.treeIdPath.length > 0) {
                                        M.downloadFileXhrImg(res.data.treeIdPath, '', M('#recieveSign'));
                                    } else {
                                        M('#recieveSign').hide();
                                    }

                                }else {
                                    M('.objection-btn').unbind('click').bind('click', function () {
                                        M.ui.waiting.creat({
                                            status:false,
                                            time:3000,
                                            text:res.message,
                        ...
                        M('.objection-btn').click(function () {
                            if (!that.isClick) {
                                ...
                                M.ajaxFn({
                                    url:  $.interfacePath.bill +'t/financeBill/pdfPrint',
                                    type: 'get',
                                    data: {
                                        billNo: that.info.billNo,
                                        num:timer
                                    },
                                    dataType: 'json',
                                    success: function (res) {
                                        if ( res.success ) {
                                            that.ajaxSubmit(that, res.data, userInfo.userId, that.info.billNo);
                                            M.ui.waiting.creat({
                                                position:'fixed',
                                                status:'loading',
                                                time:1000,
            ...
            viewSign: function (that, signNo) {
                M.ajaxFn({
                    ...
                    data: {
                        ...
                        returnUrl: 'bill/billIssueDetail.html?id='+that.info.billNo,
                        notifyUrl: 'bill/nologin/auditUser/check/asynchronousCallback'
                    },
                    dataType: 'json',
                    success: function (res) {
                        if ( res.success ) {
                ...
                M.ui.customDialog.init({
                    ...
                    callback: function (e) {
                        ...
                        M('.objection-btn').click(function () {
                            ...
                            M.ajaxFn({
                                url:M.interfacePath.bill+'t/auditUser/check',
                                type:'post',
                                dataType:'json',
                                data:{
                                    "id": id,
                    ...
                    success: function (res) {
                        ...
                        if (data == null || res.success == false||data.length ==0) {
                            ...
                            for (var i = 0; i < data.length; i++) {
                                ...
                                var statusOperator = "";
                                if (item.status == 10) {
                                    checkHtml = '<a class="ui-button ui-btn-bt ui-btn-red" href="javascript:;" onclick=\'M.billIssue.check("' + item.billNo+'","'+ item.id + '","'+ item.receivingId + '","'+ item.payerIsGroup + '");\'>复核</a>';
