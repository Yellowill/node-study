...
//				}[index]);
			...
			select:function(e){
                ...
                that.legalRepresentativeOffice=this.getAttribute('legalRepresentativeOffice');
				that.change();
				that.hide();
			...
			resolve:function(data,args){
				for(var i=0,str='';i<data.length;i++){
					str+=M.template.resolve(this.ops.tip?this.renderItemTip():this.renderItem(),{name:data[i].chineseFullName,value:data[i].comId,tax:data[i].taxNum,legalRepresentativeOffice:data[i].legalRepresentativeOffice,tax:data[i].taxNum,tip:data[i].s,className:i==0?'active':''});
				};
			...
			renderItem:function(){
				return '<li class="{{className}}" uid="{{value}}" taxNum="{{tax}}" legalRepresentativeOffice="{{legalRepresentativeOffice}}">{{name}}</li>';
			},
